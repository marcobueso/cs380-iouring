//Based on https://unixism.net/2020/04/io-uring-by-example-part-1-introduction/
...
int get_completion_and_print(struct io_uring *ring) {
    ...
    int blocks = (int) fi->file_sz / BLK_SZ;
    if (fi->file_sz % BLK_SZ) blocks++;
...
int submit_read_request(char *file_path, struct io_uring *ring) {
    ...
    off_t offset = 0;
    int current_block = 0;
    int blocks = (int) file_sz / BLK_SZ;
    if (file_sz % BLK_SZ) blocks++;
    ...
    while (bytes_remaining) {
        ...
        if (bytes_to_read > BLK_SZ)
            bytes_to_read = BLK_SZ;

        offset += bytes_to_read;
        fi->iovecs[current_block].iov_len = bytes_to_read;

        void *buf;
        if( posix_memalign(&buf, BLK_SZ, BLK_SZ)) {
